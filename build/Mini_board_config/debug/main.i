...
if(minesTouche!='0'&&m_tabMines[i][j]!=2)
m_tabMines[i][j]=minesTouche;
else if(m_tabMines[i][j]!=2)
m_tabMines[i][j]=' ';
}
}
for(unsigned char k=0;k<4;k++)
...
if(m_tabMines[xCoord][yCoord]!=2 && m_tabVue[xCoord][yCoord]!=' ')
{
...
{
if(m_tabMines[y-1][x-1]==2&&m_tabVue[y-1][x-1]!=3)
return 0;
else if(m_tabMines[y-1][x-1]!=2 && m_tabMines[y-1][x-1]!=' ')
{
...
if(m_tabMines[yTabLigne+j][xTabCol+i]!=2&&m_tabVue[yTabLigne+j][xTabCol+i]==1)
{
lcd_gotoXY(x+i,y+j);
...
for(j=0;j<20;j++)
{
if((m_tabVue[i][j]==1||m_tabVue[i][j]==3)&&m_tabMines[i][j]==2)
{
compteurBombe++;
}
else if((m_tabVue[i][j]==1||m_tabVue[i][j]==3)&&m_tabMines[i][j]!=2)
resteTuiles++;
}
}
}
